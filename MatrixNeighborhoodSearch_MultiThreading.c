...
void* neighborhood_search(void *numElementsInput) { //Method for performing neighborhood search functions
    ...
    int tempNum, m;
    if(check>0) { //Loop through the file and perform neighborhood searches until all values are either 1 or 0
      fflush(stdout); //Clear output buffer before reading matrix values
      rewind(smf); //Reset pointer to beginning of binary file
      for(m=0; m<numElements; m++) { //Loop through the binary file
        fread(&tempNum, sizeof(int), 1, smf); //Read each line
        sumNeighborhood += tempNum; //Sum neighbor values
      } //End for
      if(sumNeighborhood==0) { //Entire neighborhood is set to 0
        check = -1; //Set check to -1 to end program
        printf("Entire neighborhood is set to 0!\n"); //Print program completion
      } else if(sumNeighborhood==numElements) { //Entire neighborhood is set to 1
        check = -1; //Set check to -1 to end program
...
int thread_pooling() { //Method for creating threads to search random neighborhoods
   pthread_mutex_lock(&count_mutex);//Mutually exclude multiple threads from writing to the binary file at the same time
   fflush(stdout); //Clear output buffer before reading matrix values
   rewind(smf); //Reset pointer to beginning of binary file
   int threadNum = 0; //The current thread number
   pthread_t threadPool[numThreads]; //Thread pool structure
   for(threadNum=0;threadNum<numThreads;threadNum++){ //Loop and create threads
    ...
    pthread_mutex_unlock(&count_mutex);//Mutually exclude multiple threads from writing to the binary file at the same time
} //End thread_pooling
int main(void) { //The main method
  ...
  int mainCheck = 1; //Check variable for neighborhood state
  ...
  while(mainCheck>0) { //Loop through the file and perform neighborhood searches until all values are either 1 or 0
    ...
    if(sumNeighborhoodMatrix==0) { //Entire neighborhood is set to 0
  	  mainCheck = -1; //Set check to -1 to end program
  	  printf("Entire neighborhood is set to 0!\n"); //Print program completion
    } else if(sumNeighborhoodMatrix==numElements) { //Entire neighborhood is set to 1
      mainCheck = -1; //Set check to -1 to end program
      printf("Entire neighborhood is set to 1!\n"); //Print program completion
    } else { //Keep searching 
      thread_pooling(); //Create threads for searching neighborhoods
    } //End if, else
	} //End while
